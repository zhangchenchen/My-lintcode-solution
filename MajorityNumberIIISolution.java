...
public class MajorityNumberIIISolution {
    /**
    ...
    public static int majorityNumber(ArrayList<Integer> nums, int k) {
        ...
        for(Integer i : nums){
            if(mapHelper.containsKey(i)){
                mapHelper.put(i,mapHelper.get(i) + 1);
            }
            else{
                mapHelper.put(i,1);
            }
            if(mapHelper.size() == k){
                for(Integer j : mapHelper.keySet()){
                    mapHelper.put(j , mapHelper.get(j) - 1);
                    if(mapHelper.get(j) == 0){
                        mapHelper.remove(j);
                    }
                }
            }
